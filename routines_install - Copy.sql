 ...
 **/
   ...
   IF source IS NULL THEN 
      UPDATE vendor_queue
         SET state = 'ERROR ON DATA IMPORT',
             error_message = 'SOURCE TABLE NOT DEFINED!'
...
BEGIN
  ...
  -- Make sure the session id given is valid.
  ...
  IF successful THEN
      ...
      UPDATE vendor_queue
         SET state = 'CLEANING UP VENDOR NAMES'
       WHERE session_id = sessionid;
      COMMIT;
	...
	-- 1800 Pull Keywords
      ...
      UPDATE vendor_queue
         SET state = 'PULLING KEYWORDS FROM VENDOR NAMES'
       WHERE session_id = sessionid;
      COMMIT;
	...
	DELETE 
	 ...
	 WHERE EXISTS (
      ...
      UPDATE vendor_queue
         SET state = 'PULLING KEYWORDS FROM VENDOR NAMES STAGE 2'
       WHERE session_id = sessionid;
      COMMIT;
	...
	CALL udp_get_keywords2(theSessionId);
      ...
      UPDATE vendor_queue
         SET state = 'PERFORMING FINAL CLEANUP ON IMPORTED RECORDS'
       WHERE session_id = sessionid;
      COMMIT;
	...
	UPDATE stg_vendor_address_clean 
	 ...
	 WHERE session_id = sessionid 
      ...
      UPDATE vendor_queue
         SET state = 'TRANSFERRING DATA INTO ACTUAL TABLE'
       WHERE session_id = sessionid;
      COMMIT;
     ...
     UPDATE vendor_queue
        SET state = 'FINISHED DOING DATA IMPORT'
      WHERE session_id = sessionid;
     COMMIT;  
  ELSE
...
*/
    UPDATE vendor_queue
       SET state = 'FINISHED DATA ENTRY CHECKS'
     WHERE session_id = sessionid;
    COMMIT;  
 ...
 */
  ...
  INSERT INTO vendor_dups(session_id, group_id, sys, vend_num) SELECT sessionid, grpid, sys, vend_num FROM temp_all;
    ...
    UPDATE vendor_queue
       SET state = 'READY FOR GROUPING SIMILAR RECORDS'
     WHERE session_id = sessionid;
    COMMIT;  
 ...
 **/
  ...
  IF ISNULL(sessionid) OR sessionid='' THEN
     ...
     LEAVE body;
  ELSE
     SET sessionCount = (SELECT COUNT(DISTINCT session_id) FROM vendor_queue WHERE session_id = sessionid AND state = 'READY FOR GROUPING SIMILAR RECORDS');
     IF sessionCount <= 0 THEN
         SET result = 1;
         SET error_message = (SELECT GROUP_CONCAT(DISTINCT state) FROM vendor_queue WHERE session_id = sessionid);
         SET error_message = CONCAT_WS('','Cannot process the session_id: ', sessionid ,' because it is currently in "', error_message, '" status.');
         LEAVE body;
     END IF;
  END IF;
  
  UPDATE vendor_queue
     SET state = 'ON GROUPING PROCESS'
   WHERE session_id = sessionid;
  COMMIT;
  ...
  IF COALESCE(include_match_key, FALSE) THEN  
   ...
   SET result = 1;
   UPDATE vendor_queue
      SET state = 'FINISHED GROUPING THE VENDOR RECORDS'
    WHERE session_id = sessionid;
   COMMIT;           
