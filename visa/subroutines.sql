...
BEGIN
  INSERT INTO t05_card_transaction(
		 ...
		 supplier_city,                     
		 supplier_state_code,               
		 supplier_iso_country_code,         
		 supplier_postal_code,              
		 source_amount,                     
		 billing_amount,                    
		 source_currency_code,              
		 merchant_category_code,            
		 tansaction_type_code,              
		 transaction_date,                  
		 ...
		 purchase_id,                       
		 transaction_time,                  
		 tax_amount_incl_code,              
		 tax2_amount_incl_code,             
		 order_type_code,                   
		 message_id,                        
		 processor_addendum_key,            
		 mps_id,                            
		 usage_code,                        
		 enriched_transaction_flag,         
		 ...
		 cardholder_transaction_approval,   
		 extract_id,                        
		 memo_post_flag,                    
		 statement_date,                    
		 user_data_1,                       
		 ...
		 user_data_5,                       
		 user_data_5_desc,                  
		 visa_commerce_batch_id,            
		 visa_commerce_payment_instr_date,  
		 line_item_matched_indicator,       
		 issuer_defined_usage_code,         
		 source,                            
		 optional_field_1,                  
		 optional_field_2,                  
		 optional_field_3,                  
  ...
  SELECT CAST(col1 AS SIGNED),
         ...
         col76,
         block_id,
	 IF (CAST(col19 AS SIGNED) > 0, YEAR(STR_TO_DATE(col19, '%m%d%Y')), NULL),
	 IF (CAST(col19 AS SIGNED) > 0, QUARTER(STR_TO_DATE(col19, '%m%d%Y')), NULL),
	 IF (CAST(col19 AS SIGNED) > 0, MONTH(STR_TO_DATE(col19, '%m%d%Y')), NULL)
    FROM staging
   WHERE id > from_id AND id < to_id;      
END $$
...
BEGIN
  ...
  SELECT CAST(col1 AS SIGNED),
         ...
         CAST(col12 AS SIGNED),
         col13,
         STR_TO_DATE(col14, '%m%d%Y'),
         CAST(col15 AS SIGNED)/100,
         CAST(col16 AS SIGNED)/10000,
         col17,
         col18,
         CAST(col19 AS SIGNED)/100,
         STR_TO_DATE(col20, '%m%d%Y'),
         CAST(col21 AS SIGNED),
         col22,
         col23,
         col24,
...
BEGIN
  ...
  SELECT CAST(col1 AS SIGNED),
         ...
         col12,
         CAST(col13 AS SIGNED),
         STR_TO_DATE(col14, '%H%i%S'),
         STR_TO_DATE(col15, '%H%i%S'),
         col16, 
         col17,
         CAST(col18 AS SIGNED)/100, 
         CAST(col19 AS SIGNED)/100, 
